...
public class _10_BankApplication {
	
	//account 클래스 타입의 accountArray 배열을 선언
	//타입 마음대로 설정 가능!!
	private static _09_Account[] accountArray = new _09_Account[100]; //전역으로 설정해서 이클래스에있는 모든 메소드에서 사용가능!!!!!!!!!!!!!!!!!!!!!
	...
	public static void main(String[] args) {
		...
		private static void createAccount() {
				...
				_09_Account ac = new _09_Account(ano, owner, balance); 
		...
		private static void accountList() {
			...
			for(int i=0;i<accountArray.length;i++) {
				...
				_09_Account account = accountArray[i];
		...
		private static void deposit() {
			...
			_09_Account account = findAccount(ano);
		...
		//매개변수 String ano
		//리턴타입 : account 클래스
		private static _09_Account findAccount(String ano) {
			
			//Account 클래스 타입의 변수 선언(처리가 끝난 후 리턴 값을 추가하기 위해)
			_09_Account account = null;
